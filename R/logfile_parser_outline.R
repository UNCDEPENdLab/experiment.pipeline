...
# NOTE: to access the functions defined in this file, run "source(<filename>)"
# specification:
# keep all DATA entries except those with "Mouse:" in their Event descriptions and the first 11 (remove 11 first)
# keep all with the following in Event descriptions:
#	"New trial", "On a scale", "markerPos = " (not duplicated), "image: image =", "image_2: image =", "PITshield: image ="
parseLogFile = function(file) {
	pacman::p_load(tidyverse, dplyr)
	log_raw <- read.csv(file = file, sep = "\n", header = F)

	#TODO: remove duplicate "markerPos" lines. remove first 11 rows of Type DATA?
	log_clean = log_raw %>% separate(1, sep = "\t", into = c("Timestamp", "Type", "Event"), convert=TRUE) %>% 	# divide into columns using tab
		mutate(Type=trimws(Type), Timestamp=as.numeric(Timestamp)) %>%											# cleanup/convert types
		filter(
			   (Type=="DATA" & !grepl("Mouse:", Event)) |
				grepl(".*(New trial|markerPos =|image: image =|image_2: image =|PITshield: image =).*", Event)) %>%	# filter based on above specification
		arrange(Timestamp)

	return(tibble(log_clean))
}

parseLogFileOrg = function(file) {
