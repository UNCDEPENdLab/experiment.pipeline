...
leda.sdeco <- function(leda){ # this functions calls other functions in ep.leda_deconvolution.R
  ...
  sdeconv_output <- sdeconv_analysis(leda) # set dist0
  x <- sdeconv_output$x
  leda <- sdeconv_output$leda

  optim_output <- deconv_optimize(x, nr_iv,'sdeco')
  leda$analysis0$tau <- optim_output$xopt
  leda$analysis0$opt_history <- optim_output$opthistory
