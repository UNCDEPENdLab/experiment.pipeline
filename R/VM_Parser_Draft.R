...
vm_raw <- data.table::fread("~/github_repos/experiment.pipeline/inst/examples/070_Instrumental_2020_Feb_18_1412.csv")
log_raw <- read.csv(file = "070_Instrumental_2020_Feb_18_1412.log", sep = "\n", header = F)
...
### Metadata ---------------------------------------------------------------
metadata <- vm_raw %>% dplyr::select(`sequence #`,
                                     group, 
                                     text_15.started, 
...
vm_Ins <- vm_raw %>% dplyr::select(repA.thisN,              # Response A - Button A for reward
                                   repB.thisN,              # Response B - Button B for reward
                                   vending_machine.started, # Trial start time
                                   rewardA_image.started,
                                   rewardB_image.started,
                                   tests.thisRepN,
                                   trials.thisN,
                                   ) %>% 
...
log_clean_ins <- rbind(, log_data)
#log_clean_ins$Event_time <- as.numeric(log_clean_ins$Event_time)
log_data <- log_data %>% arrange(Event_time)
log_data <- log_data %>% separate(Event, into = c("Event_type","Outcome"), sep = ":", extra = "merge")
log_data <- log_data %>% dplyr::select(Event_time, Outcome, Event_type)
...
Vm_Ins_quizzes <- vm_raw %>% 
  dplyr::select(test_response.corr, test_question.started, test_response.keys, test_response.rt, tests.thisN) %>% na.omit() %>% 
  dplyr::rename(correct = test_response.corr) %>%
  dplyr::rename(question_start = test_question.started) %>%
  dplyr::rename(key_pressed = test_response.keys) %>%
  dplyr::rename(response_rt = test_response.rt) %>%
  dplyr::rename(test_num = tests.thisN)
