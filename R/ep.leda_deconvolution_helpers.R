...
segment_driver <- function(data, remd, sigc, segmWidth){
  ...
  signpeak <- function(data, cccrimin, cccrimax, sigc){
    minL <- matrix(, nrow <- 0, ncol <- 2)
    maxL <- c()
    output <- list(minL, maxL)
    if (length(cccrimax) == 0){
      return(output)
    }
    
    dmm <- c(data[cccrimax] - data[cccrimin[1:length(cccrimin)-1]], 
            data[cccrimax] - data[cccrimin[2:length(cccrimin)]])
  ...
  [rmdimin, rmdimax] <- get_peaks(remd) # TODO get peaks of remainder needs to be ported
  ...
  for (i in 1:length(maxL)){
    ...
    # overshoot
    oversh_data <- rep(0, size(segm_idx))
    if (i < length(maxL)){
...
sdeco_interimpulsefit <- function(leda, driver, kernel, minL, maxL){
  ...
  tonicGridSize <- leda$set$tonicGridSize_sdeco
  ...
  if (length(maxL) > 2){
    ...
    iif_idx <- c(minL[2,1], iif_idx, c(minL[length(minL),2]:(length(driver)-sr)))
  ...
  for (i in c(1:length(groundtime))){
    ...
    if (length(find(t_idx)) > 2){
      groundlevel[i] <- min(c(mean(iif_data[t_idx]),  d[time_idx[t, groundtime[i]]))
    } else { # if no inter-impulses data is available ...
      groundlevel[i] <- min(c(median(driver[grid_idx]),  d[time_idx[t, groundtime[i]]]))
    }
  }
  ...
  for (i in seq(length(groundtime), 1, by <- -1)){
  ...
  pp <- pracma::pchip(groundtime, groundlevel)
...
fiterror <- function(data, fit, npar, errortype, err_data == 0){
  ...
  error <- case_when(errortype == "MSE" ~ SSE/n, # MSE, non-normalized
                     errortype == "RMSE" ~ sqrt(SSE/n), # RMSE, non-normalized
                     errortype == "adjR2" ~ 1-(1-r2)*(n-1)/df, # adjusted-R^2; for optimization use 1 - adjR2 since you want to minimize the function
                     errortype == "Chi2" ~  SSE/err_data) # TODO where is this value coming from
...
#' @author Nidhi Desai
#'
cgd <- function(start_val, error_fcn, h, crit_error, crit_iter, crit_h){
...
while (1){
  ...
  if (any(direction)){
    # LINESEARCH
    cgd_line_output <- cgd_linesearch(x, olderror, direction, error_fcn, h)
    x <- cgd_line_output$x
    newerror <- cgd_line_output$newerror
  ...
  return(list(x = x, history = history))
}
...
#' @author Nidhi Desai
#'   
cgd_get_gradient <- function(x, error0, error_fcn, h){
...
#' @author Nidhi Desai
#' 
cgd_linesearch <- function(x, error0, direction, error_fcn, h){
  ...
  for (iStep in 2:maxSteps){
    ...
    error_fcn_output <- error_fcn(xc) # xc may be changed due to limits
    error_list[iStep] <- error_fcn_output$error
    xc <- error_fcn_output$xc
    ...
    if ((error_list[length(error_list)] >= error_list[length(error_list)-1]){ # end of decline
      if (iStep == 2){ # no success
        ...
        error_fcn_output = error_fcn(xcm) # xc may be changed due to limits
        error1 <- error_fcn$error
        xcm <- error_fcn$xc
        if (error1 < error_list[iStep-1]){
          ...
          error_fcn_output <- error_fcn(xc) # recalculate error in order to check for limits again
          error1 <- error_fcn_output$error
          xc <- error_fcn_output$xc
          step <- factor_cgd[iStep-1]
        }
      }
      return(list(xc = xc, error1 = error1, step = step))
    }
  }
