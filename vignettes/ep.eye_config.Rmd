...
edf_dir <- file.path(groupdir, "studies/NeuroMAP/s3_data/SortingMushrooms_PSU/Eye")
edf_files <- list.files(edf_dir, full.names = TRUE); print(edf_files[1:5])
edf_path <-edf_files[1] # extract a single subject for example case
config_path <- file.path(groupdir,"studies/NeuroMAP/s3_data_ep_specs/yaml/SortingMushrooms.yaml")#; print(config_path)
```
...
Importantly, these subfields constitute a task-general hierarchy that will be present regardless of the specifics of any task. An single task sits atop this hierarchy and a single config file will be needed to process each task, with phases, blocks, trials, and events all nested within a task. If you are processing a battery of tasks, we have provided documentation **[HERE]** on how to simultaneuosly process multiple tasks, but each task will need to have a set of unique preprocessing options stored in a YAML file. This vignette documents how to set up a config/YAML file for a single cognitive task, which can be easily copied over if you would like to use experiment.pipeline's batch processing capabilities.  
...
- `prefix`: A regex to append to the saved .elog and preprocessed data. If supplied, experiment.pipeline will attempt to extract the desired prefix from the basename of the .edf file using `stringr::str_extract`. **Importantly**, if passing a regex in your config file, your regex must be double quoted (rather than single) in order to keep `read_yaml` from appending additional escape characters to your string. If this is left null/empty, will simply use the `basename()` of the .edf file being processed. Defaults to NULL.
  - example of how to test if your regex works:
```{r}
## if prefix is specified in config:
prefix_regex <- "\\d{3}_[[:upper:]]+" # If all files in your edf_dir have the same naming structure, you can be assured that this regex, when applied across your edf files will extract the exact string you want to label your files as.
stringr::str_extract(edf_path, prefix_regex)
## default option: use basename()
basename(edf_path)
```
...
Initialize `ep.eye` definitions are all utilized in Step 2: Initialize ep.eye object (`ep.eye_initialize.R`). These options configure how the `.edf` file is read into an `ep.eye` object and the initial validation and data wrangling that goes into setting up a subject to be processed 
